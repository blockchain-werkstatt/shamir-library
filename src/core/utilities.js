...
const split = (secret, n, k, prime) =>{
  if (Number.isInteger(secret) || Number.isInteger(prime)) {
    throw new TypeError(
      'The split() function must be called with a String<secret>' +
  ...
  if (Number.isInteger(prime)) {
    throw new TypeError(
      'The split() function must be called with a String<prime>' +
  ...
  if (!Decimal.isDecimal(prime) && prime.substring(0, 2) !== '0x') {
    throw new TypeError(
      'The split() function must be called with a' +
...
const lagrangeInterpolate = (data, p) => {
  let S = Decimal(0);
